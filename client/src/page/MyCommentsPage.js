  ...
  const getComments = async () => {
    try {
      const response = await axios.get(
        `http://ec2-54-180-158-15.ap-northeast-2.compute.amazonaws.com:8080/board/integrated/1/comment`,
        {
          headers: {
  ...
  const deleteComment = async () => {
    try {
      const deletePromises = selectedComments.map(({ id }) => {
        return axios.delete(
          `http://ec2-54-180-158-15.ap-northeast-2.compute.amazonaws.com:8080/board/integrated/1/comment/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
