...
@RestController
@RequestMapping("/board/esfp")
public class BoardEsfpController {
    private final BoardEsfpService service;
    private final CustomBoardEsfpMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardEsfpController(BoardEsfpService service,
                               CustomBoardEsfpMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardEsfpDto.Post postDto) {
        BoardEsfp createdPost = service.createPost(mapper.postDtoToBoardEsfp(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardEsfpDto.Response result = mapper.boardEsfpToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardEsfp> pagePosts = service.findAllPost(page - 1, size);
        List<BoardEsfp> posts = pagePosts.getContent();
        List<BoardEsfpDto.Response> result = mapper.boardEsfpToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardEsfp post = service.readPost(postId);
        BoardEsfpDto.Response result = mapper.boardEsfpToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardEsfpDto.Patch patchDto) {

        BoardEsfp post = mapper.patchDtoToBoardEsfp(patchDto);
        post.setId(postId);

        BoardEsfp modifiedPost = service.updatePost(post);
        BoardEsfpDto.Response result = mapper.boardEsfpToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardEsfp().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardEsfp().getId());
