...
@RestController
@RequestMapping("/board/intp")
public class BoardIntpController {
    private final BoardIntpService service;
    private final CustomBoardIntpMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardIntpController(BoardIntpService service,
                               CustomBoardIntpMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardIntpDto.Post postDto) {
        BoardIntp createdPost = service.createPost(mapper.postDtoToBoardIntp(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardIntpDto.Response result = mapper.boardIntpToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardIntp> pagePosts = service.findAllPost(page - 1, size);
        List<BoardIntp> posts = pagePosts.getContent();
        List<BoardIntpDto.Response> result = mapper.boardIntpToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardIntp post = service.readPost(postId);
        BoardIntpDto.Response result = mapper.boardIntpToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardIntpDto.Patch patchDto) {

        BoardIntp post = mapper.patchDtoToBoardIntp(patchDto);
        post.setId(postId);

        BoardIntp modifiedPost = service.updatePost(post);
        BoardIntpDto.Response result = mapper.boardIntpToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardIntp().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardIntp().getId());
