...
@RestController
@RequestMapping("/board/intj")
public class BoardIntjController {
    private final BoardIntjService service;
    private final CustomBoardIntjMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardIntjController(BoardIntjService service,
                               CustomBoardIntjMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardIntjDto.Post postDto) {
        BoardIntj createdPost = service.createPost(mapper.postDtoToBoardIntj(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardIntjDto.Response result = mapper.boardIntjToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardIntj> pagePosts = service.findAllPost(page - 1, size);
        List<BoardIntj> posts = pagePosts.getContent();
        List<BoardIntjDto.Response> result = mapper.boardIntjToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardIntj post = service.readPost(postId);
        BoardIntjDto.Response result = mapper.boardIntjToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardIntjDto.Patch patchDto) {

        BoardIntj post = mapper.patchDtoToBoardIntj(patchDto);
        post.setId(postId);

        BoardIntj modifiedPost = service.updatePost(post);
        BoardIntjDto.Response result = mapper.boardIntjToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardIntj().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardIntj().getId());
