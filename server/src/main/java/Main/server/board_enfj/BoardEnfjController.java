...
@RestController
@RequestMapping("/board/enfj")
public class BoardEnfjController {
    private final BoardEnfjService service;
    private final CustomBoardEnfjMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardEnfjController(BoardEnfjService service,
                               CustomBoardEnfjMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardEnfjDto.Post postDto) {
        BoardEnfj createdPost = service.createPost(mapper.postDtoToBoardEnfj(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardEnfjDto.Response result = mapper.boardEnfjToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardEnfj> pagePosts = service.findAllPost(page - 1, size);
        List<BoardEnfj> posts = pagePosts.getContent();
        List<BoardEnfjDto.Response> result = mapper.boardEnfjToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardEnfj post = service.readPost(postId);
        BoardEnfjDto.Response result = mapper.boardEnfjToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardEnfjDto.Patch patchDto) {

        BoardEnfj post = mapper.patchDtoToBoardEnfj(patchDto);
        post.setId(postId);

        BoardEnfj modifiedPost = service.updatePost(post);
        BoardEnfjDto.Response result = mapper.boardEnfjToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardEnfj().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardEnfj().getId());
