...
@RestController
@RequestMapping("/board/entp")
public class BoardEntpController {
    private final BoardEntpService service;
    private final CustomBoardEntpMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardEntpController(BoardEntpService service,
                               CustomBoardEntpMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardEntpDto.Post postDto) {
        BoardEntp createdPost = service.createPost(mapper.postDtoToBoardEntp(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardEntpDto.Response result = mapper.boardEntpToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardEntp> pagePosts = service.findAllPost(page - 1, size);
        List<BoardEntp> posts = pagePosts.getContent();
        List<BoardEntpDto.Response> result = mapper.boardEntpToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardEntp post = service.readPost(postId);
        BoardEntpDto.Response result = mapper.boardEntpToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardEntpDto.Patch patchDto) {

        BoardEntp post = mapper.patchDtoToBoardEntp(patchDto);
        post.setId(postId);

        BoardEntp modifiedPost = service.updatePost(post);
        BoardEntpDto.Response result = mapper.boardEntpToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardEntp().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardEntp().getId());
