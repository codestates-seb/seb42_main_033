...
@RestController
@RequestMapping("/board/estj")
public class BoardEstjController {
    private final BoardEstjService service;
    private final CustomBoardEstjMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardEstjController(BoardEstjService service,
                               CustomBoardEstjMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardEstjDto.Post postDto) {
        BoardEstj createdPost = service.createPost(mapper.postDtoToBoardEstj(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardEstjDto.Response result = mapper.boardEstjToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardEstj> pagePosts = service.findAllPost(page - 1, size);
        List<BoardEstj> posts = pagePosts.getContent();
        List<BoardEstjDto.Response> result = mapper.boardEstjToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardEstj post = service.readPost(postId);
        BoardEstjDto.Response result = mapper.boardEstjToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardEstjDto.Patch patchDto) {

        BoardEstj post = mapper.patchDtoToBoardEstj(patchDto);
        post.setId(postId);

        BoardEstj modifiedPost = service.updatePost(post);
        BoardEstjDto.Response result = mapper.boardEstjToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardEstj().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardEstj().getId());
