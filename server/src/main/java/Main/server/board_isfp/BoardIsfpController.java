...
@RestController
@RequestMapping("/board/isfp")
public class BoardIsfpController {
    private final BoardIsfpService service;
    private final CustomBoardIsfpMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardIsfpController(BoardIsfpService service,
                               CustomBoardIsfpMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardIsfpDto.Post postDto) {
        BoardIsfp createdPost = service.createPost(mapper.postDtoToBoardIsfp(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardIsfpDto.Response result = mapper.boardIsfpToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardIsfp> pagePosts = service.findAllPost(page - 1, size);
        List<BoardIsfp> posts = pagePosts.getContent();
        List<BoardIsfpDto.Response> result = mapper.boardIsfpToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardIsfp post = service.readPost(postId);
        BoardIsfpDto.Response result = mapper.boardIsfpToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardIsfpDto.Patch patchDto) {

        BoardIsfp post = mapper.patchDtoToBoardIsfp(patchDto);
        post.setId(postId);

        BoardIsfp modifiedPost = service.updatePost(post);
        BoardIsfpDto.Response result = mapper.boardIsfpToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardIsfp().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardIsfp().getId());
