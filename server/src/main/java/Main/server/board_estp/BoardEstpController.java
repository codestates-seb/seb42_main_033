...
@RestController
@RequestMapping("/board/estp")
public class BoardEstpController {
    private final BoardEstpService service;
    private final CustomBoardEstpMapper mapper;
    private final LikeRepository likeRepository;
    private final CommentMapper commentMapper;

    public BoardEstpController(BoardEstpService service,
                               CustomBoardEstpMapper mapper,
    ...
    //게시글 등록
    @PostMapping
    public ResponseEntity postPost(@RequestBody BoardEstpDto.Post postDto) {
        BoardEstp createdPost = service.createPost(mapper.postDtoToBoardEstp(postDto), postDto.getUserId());

        if(createdPost != null) {
            BoardEstpDto.Response result = mapper.boardEstpToResponseDto(createdPost);
    ...
    public ResponseEntity getPosts(@Positive @RequestParam(defaultValue = "1") int page,
        ...
        Page<BoardEstp> pagePosts = service.findAllPost(page - 1, size);
        List<BoardEstp> posts = pagePosts.getContent();
        List<BoardEstpDto.Response> result = mapper.boardEstpToResponseDtos(posts);
    ...
    public ResponseEntity getPost(@PathVariable("post-id") long postId) {
        ...
        BoardEstp post = service.readPost(postId);
        BoardEstpDto.Response result = mapper.boardEstpToResponseDto(post);
    ...
    public ResponseEntity patchPost(@PathVariable("post-id") long postId,
                                    @RequestBody BoardEstpDto.Patch patchDto) {

        BoardEstp post = mapper.patchDtoToBoardEstp(patchDto);
        post.setId(postId);

        BoardEstp modifiedPost = service.updatePost(post);
        BoardEstpDto.Response result = mapper.boardEstpToResponseDto(modifiedPost);
    ...
    public ResponseEntity postComment(@PathVariable("post-id") Long postId,
        ...
        if(postId == postDto.getPostId()) {
            ...
            if(createComment != null) {
                ...
                result.setBoardId(createComment.getBoardEstp().getId());
    ...
    public ResponseEntity getComment(@PathVariable("post-id") long postId,
        ...
        if(post.getId() == postId && comment.getId() == commentId) {
            ...
            result.setBoardId(comment.getBoardEstp().getId());
